...
regno = 20102113001 # enter the starting registration no.

res_link = 'http://results.akuexam.net/ResultsBTechBPharm3rdSemPub2021.aspx?Sem=III&RegNo=' # enter the result link without registration no.

regno_list = []
for i in range(0,54): # enter the result range
    regno_list.append(regno)
    regno = regno + 1


regno_le = []
regno_le_var = 21102113901 # enter the starting registration no. of le students, if any. if none comment out relevant portions

for i in range (0, 16): # enter the range for the le students
...
pdfs = regno_pdf
pdfs.sort()
merge_pdfs(input_files=pdfs, page_range = (0,1), output_file = '2k19_me_3rd_sem.pdf') # enter the name for the output file in the output_file argument
